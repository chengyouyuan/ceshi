 ...
 * 创蓝发送短信
 */
public class HxdSmsProcess extends BaseSmsProcess {
	 ...
	 * 短信发送
	 *
	 * @param mobile  发信发送的目的号码.多个号码之间用英文半角逗号( , )隔开
	 * @param content 短信的内容
	...
	protected static SmsReturn sendWithReturn(String mobile, String content, String type) {
		...
		try {
			if (content != null && content.trim().length() > 0) {
				...
				naps.add(new BasicNameValuePair(SmsConstant.KEY_ACCOUNT, smsSupplier.getAccount()));
				naps.add(new BasicNameValuePair(SmsConstant.KEY_PSWD, smsSupplier.getPwd()));
				naps.add(new BasicNameValuePair(SmsConstant.KEY_MOBILE, mobile));
				naps.add(new BasicNameValuePair(SmsConstant.KEY_NEEDSTATUS, String.valueOf(SmsConstant.NEEDSTATUS)));
				naps.add(new BasicNameValuePair(SmsConstant.KEY_CONTENT, content));

				CloseableHttpClient httpClient = httpClientUtil.getHttpClient();
				HttpPost httpPost = new HttpPost(smsSupplier.getUrl() + SmsConstant.HTTPBATCH);
				httpPost.setEntity(new UrlEncodedFormEntity(naps, ContextHelper.UTF_8));
